...
readRawData <- function(type) {
  data <- data.frame(read.table(paste("none_th_", type, ".txt", sep=""), header=TRUE, fill=TRUE, colClasses = c(rep("numeric", 3), "character", "numeric")))
  data <- rbind(data, data.frame(read.table(paste("all_th_", type, ".txt", sep=""), header=TRUE, fill=TRUE, colClasses = c(rep("numeric", 3), "character", "numeric"))))
